...
def print_actor_obs(obs: np.ndarray):
    """
    ...
    console.print(
        "\n--- [bold magenta]Parsing Actor Observation[/bold magenta] ---")

    # Corrected top-level order and dimensions
    print_component("actions", 23)
    print_component("base_ang_vel", 3)
    print_component("dif_local_rigid_body_pos", 72)
    print_component("dof_pos", 23)
    print_component("dof_vel", 23)

    # Handle history_actor with its internal structure
    console.print("[cyan bold]history_actor:[/cyan bold]")
    history_actor_start_idx = current_idx
    history_actor_total_dim = 300
    ...
    print_history_sub_component("actions", 92)
    print_history_sub_component("base_ang_vel", 12)
    print_history_sub_component("dof_pos", 92)
    print_history_sub_component("dof_vel", 92)
    print_history_sub_component("projected_gravity", 12)

    current_idx += history_actor_total_dim  # Advance the main index

    # Remaining top-level components
    print_component("local_ref_rigid_body_pos", 72)
    ...
    console.print("--- [bold magenta]End Parsing[/bold magenta] ---")
