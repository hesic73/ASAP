...
class IsaacSim(BaseSimulator):
    def __init__(self, config, device):
        ...
        sim_config: SimulationCfg = SimulationCfg(
            dt=1. / self.simulator_config.sim.fps,
        ...
        logger.info(
            f"\tPhysics step-size     : {1. / self.simulator_config.sim.fps}")
        logger.info(f"\tRendering step-size   : {1. /
                                                 self.simulator_config.sim.fps *
                                                 self.simulator_config.sim.substeps}")
    ...
    def _setup_scene(self):
        ...
        # prapare to override the spawn configuration in
        # HumanoidVerse/humanoidverse/simulator/isaacsim_articulation_cfg.py
        from omni.isaac.lab.utils.assets import ISAACLAB_NUCLEUS_DIR
        ...
        init_state = ArticulationCfg.InitialStateCfg(
            ...
            joint_pos={
                joint_name: joint_angle for joint_name,
                joint_angle in default_joint_angles.items()},
            joint_vel={
                ".*": 0.0},
        )
                ...
                if proportion > 0:
                    if terrain_type == "flat":
                        ...
                        sub_terrains[terrain_type] = terrain_gen.HfRandomUniformTerrainCfg(
                            proportion=proportion, noise_range=(
                                0.02, 0.10), noise_step=0.02, border_width=0.25)
                    elif terrain_type == "low_obst":
                        sub_terrains[terrain_type] = terrain_gen.MeshRandomGridTerrainCfg(
                            proportion=proportion, grid_width=0.45, grid_height_range=(
                                0.05, 0.2), platform_width=2.0)
    ...
    def load_assets(self):
        ...
        # warning if the dof_ids order does not match the joint_names order in
        # robot_config
    ...
    def get_dof_limits_properties(self):
        ...
        for i in range(self.num_dof):
            self.hard_dof_pos_limits[i,
                                     0] = self.robot_config.dof_pos_lower_limit_list[i]
            self.hard_dof_pos_limits[i,
                                     1] = self.robot_config.dof_pos_upper_limit_list[i]
    ...
    def refresh_sim_tensors(self):
        ...
        self._rigid_body_rot = self._robot.data.body_quat_w[:, self.body_ids][:, :, [
            1, 2, 3, 0]]  # (num_envs, 4) 3 isaacsim use wxyz, we keep xyzw for consistency
    ...
    def set_dof_state_tensor(self, set_env_ids, dof_states):
        dof_pos, dof_vel = dof_states[set_env_ids,
                                      :, 0], dof_states[set_env_ids, :, 1]
    ...
    def simulate_at_each_physics_step(self):
        ...
        # If a camera needs rendering at a faster frequency, this will lead to
        # unexpected behavior.
