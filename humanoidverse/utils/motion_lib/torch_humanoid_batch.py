...
class Humanoid_Batch:
    def __init__(self, cfg, device=torch.device("cpu")):
        ...
        tree = parse(BytesIO(open(self.mjcf_file, "rb").read()),
                     parser=parser,)
        self.dof_axis = []

        joints = sorted([j.attrib['name'] for j in tree.getroot().find(
            "worldbody").findall('.//joint')])
        motors = sorted([m.attrib['name']
        ...
        self._offsets: torch.Tensor = mjcf_data['local_translation'][None, ].to(
            device)
        self._local_rotation: torch.Tensor = mjcf_data['local_rotation'][None, ].to(
            device)
        self.actuated_joints_idx: np.ndarray = np.array(
        ...
        self._local_rotation_mat = quaternion_to_matrix(
            self._local_rotation).float()  # w, x, y ,z
        self.load_mesh()
    ...
    def from_mjcf(self, path):
        ...
        def _add_xml_node(xml_node, parent_index, node_index):
            ...
            pos = np.fromstring(xml_node.attrib.get(
                "pos", "0 0 0"), dtype=float, sep=" ")
            quat = np.fromstring(xml_node.attrib.get(
            ...
            for joint in all_joints:
                if not joint.attrib.get("range") is None:
                    joints_range.append(np.fromstring(
    ...
    def forward_kinematics_batch(self, rotations: torch.Tensor, root_rotations: torch.Tensor, root_positions: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        expanded_offsets = (self._offsets[:, None].expand(
    ...
    def load_mesh(self):
        ...
        all_mesh_nodes = xml_assets.findall(".//mesh")
        ...
        self.geom_transforms_per_body: Dict[str,
                                            List[Dict[str, np.ndarray]]] = defaultdict(list)
        self.body_to_mesh: Dict[str, Set[str]] = defaultdict(set)
        self.mesh_to_body: Dict[ETree.Element, ETree.Element] = {}
        ...
        for geom_node in track(geoms, description="Loading Geoms..."):
            if 'mesh' in geom_node.attrib:
                ...
                if parent is None or 'name' not in parent.attrib:
                    continue  # 跳过没有父级或父级没有名字的geom

                parent_name = parent.attrib['name']
                self.body_to_mesh[parent_name].add(geom_node.attrib['mesh'])
                self.mesh_to_body[geom_node] = parent
    ...
    def mesh_fk(self, pose: Optional[torch.Tensor] = None, trans: Optional[torch.Tensor] = None):
        ...
        """
        if pose is None:
            fk_res = self.fk_batch(torch.zeros(
                1, 1, len(self.body_names_augment), 3), torch.zeros(1, 1, 3))
        else:
            fk_res = self.fk_batch(pose, trans)

        g_trans: torch.Tensor = fk_res.global_translation.squeeze(0).squeeze(0)
        g_rot: torch.Tensor = fk_res.global_rotation_mat.squeeze(0).squeeze(0)

        geoms = self.tree.find("worldbody").findall(".//geom")
        joined_mesh_obj = []

        # 使用计数器来追踪每个body下的geom索引
        geom_counters = defaultdict(int)

        for geom in geoms:
            if 'mesh' not in geom.attrib or geom not in self.mesh_to_body:
                continue
            ...
            # 2. 获取父body的全局位姿
            body_name = self.mesh_to_body[geom].attrib['name']
            body_idx = self.body_names.index(body_name)

            body_trans = g_trans[body_idx].numpy().copy()
            body_rot_mat = g_rot[body_idx].numpy().copy()

            # 3. 使用计数器获取当前geom正确的局部变换
            local_geom_idx = geom_counters[body_name]
            local_transform = self.geom_transforms_per_body[body_name][local_geom_idx]

            local_pos = local_transform['pos']
            local_quat_wxyz = local_transform['quat']
            local_quat_xyzw = local_quat_wxyz[[1, 2, 3, 0]]
            local_rot_mat = sRot.from_quat(local_quat_xyzw).as_matrix()

            # 4. 组合变换
            final_rot_mat = body_rot_mat @ local_rot_mat
            final_trans = body_trans + (body_rot_mat @ local_pos)

            # 5. 对mesh应用最终变换
            mesh_obj.rotate(final_rot_mat, center=(0, 0, 0))
            mesh_obj.translate(final_trans)
            joined_mesh_obj.append(mesh_obj)

            # 6. 为该body的geom计数器加一
            geom_counters[body_name] += 1

        if not joined_mesh_obj:
            logger.error(
                "No meshes found to merge. Please check your MJCF file.")
...
def main(config: DictConfig):
    ...
    mesh = humanoid_fk.mesh_fk()
    mesh.compute_vertex_normals()
    o3d.io.write_triangle_mesh(
        f"combined_{config.robot.motion.humanoid_type}.stl", mesh)
