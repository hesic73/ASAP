...
class MotionLibBaseOptimized():
    ...
    def _preload_data(self, data_path: str):
        ...
        if len(motion_files) != 1:
            logger.warning(
                f"Found {len(motion_files)} motion files, but this optimized version expects exactly 1. Using the first one.")
        ...
        logger.info(
            f"Pre-loaded single motion with {pose_aa.shape[0]} frames at {motion_data['fps']} fps.")
    ...
    def load_motions(self,
        ...
        logger.info(
            f"Processed single motion with length {motion_length:.3f}s and {num_frames} frames.")
        logger.info(
            f"Precomputed {len(self.discrete_times)} discrete time steps with step size {self.time_step_size}s.")
    ...
    def _precompute_discrete_states(self):
        ...
        self.discrete_times = torch.arange(
            num_steps, device=self._device, dtype=torch.float32) * self.time_step_size
        ...
        for i, time_val in enumerate(self.discrete_times):
            ...
            # DOF states
            dof_vel = (1.0 - blend_tensor) * \
                self.raw_dvs[f0] + blend_tensor * self.raw_dvs[f1]
            dof_pos = (1.0 - blend_tensor) * \
                self.raw_dof_pos[f0] + blend_tensor * self.raw_dof_pos[f1]

            # Global states
            rg_pos_t = (1.0 - blend_exp2) * \
                self.raw_gts_t[f0] + blend_exp2 * self.raw_gts_t[f1]
            rg_rot_t = slerp(self.raw_grs_t[f0].unsqueeze(
                0), self.raw_grs_t[f1].unsqueeze(0), blend_exp2.unsqueeze(0)).squeeze(0)
            body_vel_t = (1.0 - blend_exp2) * \
                self.raw_gvs_t[f0] + blend_exp2 * self.raw_gvs_t[f1]
            body_ang_vel_t = (1.0 - blend_exp2) * \
                self.raw_gavs_t[f0] + blend_exp2 * self.raw_gavs_t[f1]
        ...
        self.precomputed_states = {
            ...
            # [num_steps, 27, 3]
            'rg_pos_t': torch.stack(rg_pos_t_list, dim=0),
            # [num_steps, 27, 4]
            'rg_rot_t': torch.stack(rg_rot_t_list, dim=0),
            # [num_steps, 27, 3]
            'body_vel_t': torch.stack(body_vel_t_list, dim=0),
            # [num_steps, 27, 3]
            'body_ang_vel_t': torch.stack(body_ang_vel_t_list, dim=0),
            # [num_steps, 2]
            'foot_contacts': torch.stack(foot_contacts_list, dim=0),
        }
    ...
    def get_motion_state(self, motion_ids: torch.Tensor, motion_times: torch.Tensor, offset: Optional[torch.Tensor] = None, xy_scale: Optional[torch.Tensor] = None):
        ...
        time_indices = torch.clamp(
            time_indices, 0, len(self.discrete_times) - 1)
        ...
        if offset is not None:
            # [batch, 1, 3] -> [batch, 27, 3]
            final_rg_pos_t = final_rg_pos_t + offset.unsqueeze(1)

        # Apply xy scaling if provided
        if xy_scale is not None:
            # Get initial xy positions for all links
            initial_xy = self.initial_xy_position.unsqueeze(
                0).expand(batch_size, -1, -1)  # [batch, 27, 2]
            ...
            if xy_scale.dim() == 1:
                # [batch,] -> [batch, 2]
                xy_scale = xy_scale.unsqueeze(-1).expand(-1, 2)
    ...
    def get_motion_num_steps(self, motion_ids: Optional[torch.Tensor] = None) -> torch.Tensor:
        steps = int((self._motion_num_frames * self._sim_fps /
                    self._motion_fps).ceil().item())
