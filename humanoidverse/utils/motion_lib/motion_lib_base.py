...
class MotionLibBase():
    ...
    def _preload_data(self, data_path: str):
        ...
        for f in track(
                motion_files,
                description="Pre-loading motions into memory..."):
            data = joblib.load(f)
    ...
    def load_motions(self,
        ...
        logger.info(f"Sampling motion indices: {
                    sample_idxes[:5].cpu().numpy()}, ....")

        sampled_motion_data = [self._motion_data_cache[i]
                               for i in sample_idxes.cpu().numpy()]

        for curr_file_data in track(
                sampled_motion_data,
                description="Processing motions..."):
            ...
            trans = to_torch(
                curr_file_data['root_trans_offset']).clone()[
                start:end]
        ...
        logger.info(
            f"Processed {
                num_motions:d} motions with a total length of {
                total_len:.3f}s and {
                self.gts.shape[0]} frames.")

    def get_total_length(self) -> float:
        # This method remains unchanged from the original
        return self._motion_lengths.sum().item()

    def get_motion_num_steps(
            self, motion_ids: Optional[torch.Tensor] = None) -> torch.Tensor:
        # This method remains unchanged from the original
        if motion_ids is None:
            return (
                self._motion_num_frames *
                self._sim_fps /
                self._motion_fps).ceil().int()
        else:
            return (
                self._motion_num_frames[motion_ids] *
                self._sim_fps /
                self._motion_fps).ceil().int()
