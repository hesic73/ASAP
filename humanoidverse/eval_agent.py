...
def main(override_config: OmegaConf):
    ...
    exported_policy_path = os.path.join(
        HV_ROOT_DIR, checkpoint_dir, 'exported')
    ...
    if EXPORT_POLICY:
        export_policy_as_jit(algo.alg.actor_critic,
                             exported_policy_path, exported_policy_name)
        logger.info('Exported policy as jit script to: ', os.path.join(
            exported_policy_path, exported_policy_name))
    if EXPORT_ONNX:
