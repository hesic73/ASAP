    ...
    def _pre_compute_observations_callback(self):
        ...
        rotated_pos_in_parent = my_quat_rotate(
            self.simulator._rigid_body_rot[:,
                                           self.extend_body_parent_ids].reshape(-1, 4),
        ...
        extend_curr_rot = quat_mul(self.simulator._rigid_body_rot[:, self.extend_body_parent_ids].reshape(-1, 4),
                                   self.extend_body_rot_in_parent_xyzw.reshape(
                                       -1, 4),
        ...
        angular_velocity_contribution = torch.cross(
            self._rigid_body_ang_vel_global, self.extend_body_pos_in_parent.view(self.num_envs, -1, 3), dim=2)
        ...
        self.dif_global_body_rot = quat_mul(ref_body_rot_extend, quat_conjugate(
        ...
        global_ref_rigid_body_pos = ref_body_pos_extend.view(
            ...
            env_batch_size, -1, 3) - self.simulator.robot_root_states[:, :3].view(env_batch_size, 1, 3)
        ...
        # hard coded 1.05 because +1 will exceed 1
