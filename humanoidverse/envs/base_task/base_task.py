...
class BaseTask():
    ...
    def reset_all(self):
        ...
        self.simulator.set_actor_root_state_tensor(torch.arange(
            self.num_envs, device=self.device), self.simulator.all_root_states)
        self.simulator.set_dof_state_tensor(torch.arange(
            self.num_envs, device=self.device), self.simulator.dof_state)
        # self._refresh_env_idx_tensors(torch.arange(self.num_envs, device=self.device))
        actions = torch.zeros(self.num_envs, self.dim_actions,
    ...
    def _get_env_origins(self):
        ...
        if self.config.terrain.mesh_type in ["heightfield", "trimesh"]:
            ...
            self.terrain_types = torch.div(torch.arange(self.num_envs, device=self.device), (
                self.num_envs/self.config.terrain.num_cols), rounding_mode='floor').to(torch.long)
            ...
            if isinstance(self.simulator.terrain.env_origins, np.ndarray):
                self.terrain_origins = torch.from_numpy(
                    self.simulator.terrain.env_origins).to(self.device).to(torch.float)
            else:
    ...
    def _load_assets(self):
        ...
        base_init_state_list = self.config.robot.init_state.pos + self.config.robot.init_state.rot + \
            self.config.robot.init_state.lin_vel + self.config.robot.init_state.ang_vel
    ...
    def _setup_robot_body_indices(self):
        ...
        if self.config.robot.has_upper_body_dof:
            ...
            self.waist_dof_indices = [self.dof_names.index(
                dof) for dof in self.config.robot.waist_dof_names]
