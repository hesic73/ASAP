...
class LeggedRobotLocomotion(LeggedRobotBase):
    def __init__(self, config, device):
        ...
        self.upper_left_arm_dof_indices = [self.dof_names.index(
            dof) for dof in self.upper_left_arm_dof_names]
        self.upper_right_arm_dof_indices = [self.dof_names.index(
            dof) for dof in self.upper_right_arm_dof_names]
        self.hips_dof_id = [
            self.simulator._body_list.index(link) -
            1 for link in self.config.robot.motion.hips_link]  # Yuanhang: -1 for the base link (pelvis)
        self.init_done = True
    ...
    def _init_gait_params(self):
        ...
        self.left_feet_height = torch.zeros(
            self.num_envs, device=self.device)  # left feet height
        self.right_feet_height = torch.zeros(
            self.num_envs, device=self.device)  # right feet height
        ...
        if hasattr(self.config.rewards, "gait_period"):
            if not self.config.rewards.gait_period:
                ...
                self.T = self.config.rewards.gait_period
            else:
        ...
        self.swing_arm_joint_pos = torch.tensor(
            [-1.04, 0.0, 0.0, 1.57, 0.0, 0.0, 0.0], device=self.device, dtype=torch.float, requires_grad=False)
    ...
    def _update_tasks_callback(self):
        ...
        if not self.is_evaluating:
            env_ids = (self.episode_length_buf %
                       int(self.config.locomotion_command_resampling_time /
    ...
    def _resample_commands(self, env_ids):
        ...
        self.commands[env_ids,
                      :2] *= (torch.norm(self.commands[env_ids,
                                                       :2],
                                         dim=1) > 0.2).unsqueeze(1)
    ...
    def set_is_evaluating(self, command=None):
        ...
        if command is not None:
            self.commands[:, :3] = torch.tensor(command).to(
                self.device)  # only set the first 3 commands
    ...
    def _reward_tracking_lin_vel(self):
        ...
        return torch.exp(-lin_vel_error /
                         self.config.rewards.reward_tracking_sigma.lin_vel)
    ...
    def _reward_tracking_ang_vel(self):
        ...
        return torch.exp(-ang_vel_error /
                         self.config.rewards.reward_tracking_sigma.ang_vel)
    ...
    def _reward_penalty_ang_vel_xy_torso(self):
        ...
        torso_ang_vel = quat_rotate_inverse(
            self.simulator._rigid_body_rot[:, self.torso_index], self.simulator._rigid_body_ang_vel[:, self.torso_index])
    ...
    def _reward_penalty_feet_contact_forces(self):
        ...
        return torch.sum(
            (torch.norm(
                ...
                dim=-
                1) -
                self.config.rewards.locomotion_max_contact_force).clip(
                min=0.),
            dim=1)
    ...
    def _reward_base_height(self):
        ...
        return torch.square(base_height -
                            self.config.rewards.desired_base_height)
    ...
    def _reward_penalty_feet_height(self):
        ...
        # [num_env], # select the foot closer to target
    ...
    def _reward_penalty_feet_swing_height(self):
        contact = torch.norm(
            self.simulator.contact_forces[:, self.feet_indices, :3], dim=2) > 1.
    ...
    def _reward_penalty_close_feet_xy(self):
        ...
        left_foot_xy = self.simulator._rigid_body_pos[:,
                                                      self.feet_indices[0], :2]
        right_foot_xy = self.simulator._rigid_body_pos[:,
                                                       self.feet_indices[1], :2]
    ...
    def _reward_penalty_close_knees_xy(self):
        ...
        left_knee_xy = self.simulator._rigid_body_pos[:,
                                                      self.knee_indices[0], :2]
        right_knee_xy = self.simulator._rigid_body_pos[:,
                                                       self.knee_indices[1], :2]
        self.knee_distance_xy = torch.norm(left_knee_xy - right_knee_xy, dim=1)
        return (self.knee_distance_xy <
                self.config.rewards.close_knees_threshold) * 1.0
    ...
    def _reward_upperbody_joint_angle_freeze(self):
        ...
        deviation = torch.abs(self.simulator.dof_pos[:,
                                                     self.upper_dof_indices] - self.default_dof_pos[:,
    ...
    def _calc_phase_time(self):
        ...
        phase_time = (episode_length_np * self.dt +
                      self.phi_offset) % self.T / self.T
        return phase_time
    ...
    def _reward_contact(self):
        ...
        for i in range(2):  # left and right feet
            ...
            contact = self.simulator.contact_forces[:,
                                                    self.feet_indices[i], 2] > 1
    ...
    def _reward_gait_period(self):
        ...
        E_I_r_stance = torch.tensor(
            ...
            dtype=torch.float,
            requires_grad=False)
        # Get the contact forces and velocities of the feet, and the velocities
        # of the arm ee
