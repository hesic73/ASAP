...
class HistoryHandler:
    ...
    def __init__(self, num_envs: int, history_config: DictConfig,
                 obs_dims: Dict[str, int], device: torch.device):
    ...
    def reset(self, reset_ids: torch.Tensor):
        if len(reset_ids) == 0:
            return
        assert set(
            self.buffer_config.keys()) == set(
            self.history.keys()), f"History keys mismatch\n{
