...
class Terrain:
    ...
    def randomized_terrain(self):
        proportions = np.array(self.cfg.terrain_proportions) / \
            np.sum(self.cfg.terrain_proportions)
        for k in range(self.cfg.num_sub_terrains):
            print('generating randomized terrains %d / %d     ' %
                  (k, self.cfg.num_sub_terrains), end='\r')
    ...
    def curriculum_terrain(self):
        ...
        for terrain_type, col_range in sub_terrain_dict.items():
            print(
                'generating curriculum terrains %s    ' %
                (terrain_type), end='\r')
    ...
    def gap_terrain_func(self, terrain, difficulty):
        ...
        terrain.height_field_raw[center_x -
                                 x1: center_x +
                                 x1, center_y -
                                 y1: center_y +
                                 y1] = 0
    ...
    def rough_terrain_func(self, terrain, difficulty):
        ...
        terrain.height_field_raw = np.random.uniform(
            -max_height * 2 - 0.025, -0.025, terrain.height_field_raw.shape) / terrain.vertical_scale
    ...
    def smooth_slope_terrain_func(self, terrain, difficulty):
        ...
        terrain_utils.pyramid_sloped_terrain(
            terrain, slope=slope, platform_size=0.)
    ...
    def low_obst_terrain_func(self, terrain, difficulty):
        ...
        for i in range(obst_num):
            terrain.height_field_raw[xs[i]:xs[i] + obst_size, ys[i]
                :ys[i] + obst_size] = -max_height / terrain.vertical_scale
    ...
    def maze_terrain_func(self, terrain, difficulty):
        ...
        midroom_size = int(
            2.01 / terrain.horizontal_scale / 2) + path_width // 2
        center_x = terrain.length // 2
        center_y = terrain.width // 2

        y_low = np.random.randint(-center_y, center_y -
                                  path_width, terrain.length)
        y_high = np.random.randint(-center_y,
                                   center_y - path_width,
                                   terrain.length)
        y_low, y_high = np.minimum(
            y_low, y_high), np.maximum(
            y_low, y_high) + path_width
        y_low[center_x - midroom_size:center_x + midroom_size] = - midroom_size
        y_high[center_x - midroom_size:center_x +
               midroom_size] = + midroom_size
        ...
        for _col in range(0, terrain.length, path_width):
            ...
            terrain.height_field_raw[_col:_col +
                                     path_width, center_y +
                                     y_low[_col]:center_y +
                                     y_high[_col]] = 0.
        terrain.height_field_raw[:room_size, center_y -
                                 room_size:center_y +
                                 room_size] = 0.
        terrain.height_field_raw[-room_size:,
                                 center_y - room_size:center_y + room_size] = 0.
