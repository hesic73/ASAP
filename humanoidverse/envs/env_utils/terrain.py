...
class Terrain:
    ...
    def curriculum_terrain(self):
        ...
        for terrain_type, col_range in sub_terrain_dict.items():
            print('generating curriculum terrains %s    ' %
                  (terrain_type), end='\r')
    ...
    def maze_terrain_func(self, terrain, difficulty):
        ...
        y_high = np.random.randint(-center_y,
                                   center_y-path_width, terrain.length)
        y_low, y_high = np.minimum(y_low, y_high), np.maximum(
        ...
        for _col in range(0, terrain.length, path_width):
            ...
            terrain.height_field_raw[_col:_col+path_width,
                                     center_y+y_low[_col]:center_y+y_high[_col]] = 0.
