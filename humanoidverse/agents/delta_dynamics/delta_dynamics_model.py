...
class DeltaDynamicsModel(BaseAlgo):
    ...
    def load(self, path):
        ...
        self.delta_dynamics_optimizer.load_state_dict(
            checkpoint['delta_dynamics_optimizer'])
    ...
    def _setup_models_and_optimizer(self):
        ...
        self.delta_dynamics.to(self.device)
        self.delta_dynamics_optimizer = torch.optim.Adam(
            self.delta_dynamics.parameters(), lr=1e-3)
    ...
    def learn(self):
        ...
        for it in range(self.num_learning_iterations):
            ...
            for i in range(self.num_steps_per_env):
                ...
                delta_state_items = self.env.parse_delta(
                    pred_delta.clone(), 'pred')

                obs_dict, rew_buf, reset_buf, extras = self.env.step(
                ...
                target_state_items = self.env.parse_delta(
                    target.clone(), 'target')
                # calculate losses for different components
                loss_dof_pos = self.delta_dynamics_loss(
                    pred_state['dof_pos'], target_state_items['motion_dof_pos'])
                loss_dof_vel = self.delta_dynamics_loss(
                    pred_state['dof_vel'], target_state_items['motion_dof_vel'])
                loss_base_pos_xyz = self.delta_dynamics_loss(
                    pred_state['base_pos_xyz'], target_state_items['motion_base_pos_xyz'])
                loss_base_lin_vel = self.delta_dynamics_loss(
                    pred_state['base_lin_vel'], target_state_items['motion_base_lin_vel'])
                loss_base_ang_vel = self.delta_dynamics_loss(
                    pred_state['base_ang_vel'], target_state_items['motion_base_ang_vel'])
                loss_base_quat = self.delta_dynamics_loss(
                    pred_state['base_quat'], target_state_items['motion_base_quat'])

                loss = loss_dof_pos + loss_dof_vel + loss_base_pos_xyz + \
            ...
            self.writer.add_scalar('Loss_base_pos_xyz',
                                   loss_base_pos_xyz.item(), it)
            self.writer.add_scalar('Loss_base_lin_vel',
                                   loss_base_lin_vel.item(), it)
            self.writer.add_scalar('Loss_base_ang_vel',
                                   loss_base_ang_vel.item(), it)
    ...
    def _pre_eval_env_step(self, actor_state: dict):
        ...
        pred_delta = self.delta_dynamics(
            actor_state["obs"]['delta_dynamics_input_obs'])
    ...
    def evaluate_policy(self):
        ...
        init_delta_dynamics_input_obs = torch.zeros(
            self.env.num_envs, self.input_dim, device=self.device)
        actor_state.update({"obs": obs_dict, "actions": init_actions, "on_policy": True,
                           "delta_dynamics_input_obs": init_delta_dynamics_input_obs})
