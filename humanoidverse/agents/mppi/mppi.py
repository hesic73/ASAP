...
class PathIntegral(BaseAlgo):
    ...
    def __init__(self,
        ...
        self.noise_scheduler = {
            "cos": CosAnnealScheduler,
            "linear": LinearAnnealScheduler,
            "exp": ExpAnnealScheduler}[
            self.config.noise_scheduler.type](
            x=self.config.init_std,
            T=self.config.num_iters,
            config=self.config.noise_scheduler)
    ...
    def _get_rollouts(
        ...
        rews = torch.zeros(
            (self.num_samples,
             self.horizon),
            dtype=rollout_acts.dtype,
            device=rollout_acts.device)
        last_obs = init_state["obs"]
        states = torch.zeros((self.num_samples,
                              self.horizon + 1,
                              last_obs.shape[-1]),
                             dtype=rollout_acts.dtype,
                             device=rollout_acts.device)
    ...
    def _init_trajectory(self, init_state):
        ...
        if self.policy_prior is not None:
            _, init_mean, _ = self._get_rollouts(
                init_state=init_state, rollout_acts=None, rollout_policy=self.policy_prior)
            ...
            init_mean[:, -self.apply_plan_step:] = self.prev_plan[-1].unsqueeze(0).unsqueeze(
                0).repeat(self.num_samples, self.apply_plan_step, 1)  # keep last action
            init_std = self.config.init_std * torch.clamp(
                torch.arange(
                    1,
                    self.horizon + 1,
                    1,
                    device=self.device).unsqueeze(0).unsqueeze(
                    -1) / self.horizon,
                0.1,
                1.0)
        else:
    ...
    def planning(self, init_state, init_buf=None, step=None):
        ...
        for _ in tbar:
            ...
            tbar.set_postfix({"max_return": rets.max().item(),
                              ...
                              "noise std": std})
            if rets.mean() > self.config.plan_end_ret_mean * \
                    self.horizon or rets.max() > self.config.plan_end_ret_max * self.horizon:
    ...
    def act_process(self, manager_dict, log_queue):
        ...
        try:
            ...
            while True:
                ...
                state = {"dof_states": torch.frombuffer(buffered_state[8: 8 +
                                                                       env_num_dof *
                                                                       2 *
                                                                       8], dtype=torch.float64).to(torch.float32).view(1, env_num_dof, 2).to(self.device), "root_states": torch.frombuffer(buffered_state[8 +
                                                                                                                                                                                                          env_num_dof *
                                                                                                                                                                                                          2 *
                                                                                                                                                                                                          8: 8 +
                                                                                                                                                                                                          env_num_dof *
                                                                                                                                                                                                          ...
                                                                                                                                                                                                          8 +
                                                                                                                                                                                                          env_root_state_dim *
                                                                                                                                                                                                          8], dtype=torch.float64).to(torch.float32).view(1, -
                                                                                                                                                                                                                                                          1).to(self.device), "obs": torch.frombuffer(buffered_state[8 +
                                                                                                                                                                                                                                                                                                                     env_num_dof *
                                                                                                                                                                                                                                                                                                                     2 *
                                                                                                                                                                                                                                                                                                                     8 +
                                                                                                                                                                                                                                                                                                                     env_root_state_dim *
                                                                                                                                                                                                                                                                                                                     8:], dtype=torch.float64).to(torch.float32).view(1, obs_dim).to(self.device)}  # use real states for simulation world model
    ...
    def eval_process(self, manager_dict, log_queue):
        ...
        self.env = instantiate(
            ...
            device=self.device)  # create evaluation environment
        ...
        try:
            try:
                ...
                while not done:
                    ...
                    for k, v in curr_buf.items():
                        struct.pack_into(
                            ...
                            *
                            v.to(
                                torch.float64).cpu().numpy().flatten())  # pack buffer data
                        buf_size += v.numel() * 8
                    buf_buffer[:] = buf_data
                    ...
                    for i in range(self.apply_plan_step):
                        ...
                        t_curr = time.time() - t0
                        step += 1
                        print(f"at step {step} cumulated reward {
                              cum_rew}; planning time {t_curr:.2f} seconds")
                        log_queue.put(
                ...
                if self.config.log_actions:
                    pkl.dump(stored_actions, open(f"{self.log_dir}/data/stored_actions_{
                             self.config.eval_steps}step_{self.config.command}.pkl", "wb"))
                if self.config.log_states:
                    pkl.dump(stored_states, open(f"{self.log_dir}/data/stored_states_{
                             self.config.eval_steps}step_{self.config.command}.pkl", "wb"))
    ...
    def evaluate_policy(self, wandb_config=None):
        with Manager() as manager:
            ...
            eval_proc = Process(
                target=self.eval_process, args=(
                    manager_dict, log_queue))
            act_proc = Process(
                target=self.act_process, args=(
                    manager_dict, log_queue))
            log_proc = Process(
                target=self.log_process, args=(
