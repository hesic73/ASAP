...
def cma_es_update(weights, trajs, mean, std):
    ...
    error = trajs - mean
    std_new = torch.sqrt(torch.einsum(
        "n,nij->ij", weights, error**2)).mean() * std
...
class CosAnnealScheduler:
    ...
    def step(self, step=None):
        self.t += 1
        self.x = self.min + (self.max - self.min) * \
            (1 + np.cos(self.t * np.pi / self.T)) / 2
        return self.x
...
class PathIntegral(BaseAlgo):
    ...
    def _get_dynamics(self):
        ...
        if wm_config.type == "sim":
            ...
            self.dynamic_model = SimWorldModel(
                wm_config, self.env, self.num_samples, self.device)
            ...
            self.dynamic_model = MultiSimWorldModel(
                wm_config, sim_config, self.num_samples, self.config.command, self.device)
        else:
    ...
    def _get_rollouts(self, init_state, init_buf=None, rollout_acts=None, rollout_policy=None):
        ...
        if rollout_acts is None:
            ...
            rollout_acts = torch.zeros(
                (self.num_samples, self.horizon, self.act_dim), dtype=torch.float32, device=self.device)

        rews = torch.zeros((self.num_samples, self.horizon),
                           dtype=rollout_acts.dtype, device=rollout_acts.device)
        last_obs = init_state["obs"]
        states = torch.zeros((self.num_samples, self.horizon + 1,
                             last_obs.shape[-1]), dtype=rollout_acts.dtype, device=rollout_acts.device)
        ...
        for t in range(self.horizon):
            ...
            next_state, rew, not_done = self.dynamic_model.step(
                rollout_acts[:, t])
    ...
    def _update_once(self, init_state, init_buf, mean, std, step=None):
        epsilon = torch.randn(
            (self.num_samples, self.horizon, self.act_dim), device=self.device) * std
    ...
    def _init_trajectory(self, init_state):
        ...
        if self.policy_prior is not None:
            ...
            init_mean[:, -self.apply_plan_step:] = self.prev_plan[-1].unsqueeze(0).unsqueeze(
                # keep last action
                0).repeat(self.num_samples, self.apply_plan_step, 1)
            init_std = self.config.init_std * torch.clamp(torch.arange(
                1, self.horizon+1, 1, device=self.device).unsqueeze(0).unsqueeze(-1) / self.horizon, 0.1, 1.0)
        else:
            init_mean = torch.zeros(
                (self.num_samples, self.horizon, self.act_dim), device=self.device)
            init_std = self.config.init_std * \
                torch.ones((self.act_dim,), device=self.device)
        return init_mean, init_std

    def planning(self, init_state, init_buf=None, step=None):
        mean, std = self._init_trajectory(init_state)
        tbar = tqdm(range(self.config.num_iters),
                    desc="Processing", leave=True)
        anneal_steps = 0
        for _ in tbar:
            rets, mean, std = self._update_once(
                init_state, init_buf, mean, std, step)
            anneal_steps += 1
    ...
    def act_process(self, manager_dict, log_queue):
        ...
        init_buf = self.dynamic_model.env.get_mppi_buffers(
            [0])  # get initial buffer for sim envs
        ...
        try:
            ...
            while True:
                ...
                state = {"dof_states": torch.frombuffer(buffered_state[8: 8 + env_num_dof * 2 * 8], dtype=torch.float64).to(torch.float32).view(1, env_num_dof, 2).to(self.device),
                         ...
                         # use real states for simulation world model
                         "obs": torch.frombuffer(buffered_state[8 + env_num_dof * 2 * 8 + env_root_state_dim * 8:], dtype=torch.float64).to(torch.float32).view(1, obs_dim).to(self.device)}
                buffered_buf = buf_buffer[:]
                tgt_buf = {}
                buf_size = 0
                for k, v in init_buf.items():
                    tgt_buf[k] = torch.frombuffer(
                        buffered_buf[buf_size: buf_size+v.numel() * 8], dtype=torch.float64).view(v.shape).to(self.device)
                    buf_size += v.numel() * 8

                # plannning
                ctrl_time = t_real  # TODO: get time from state
                curr_plan, curr_info = self.planning(state, tgt_buf, step)
                ctrl_act = curr_plan[:self.apply_plan_step].to(
                    torch.float64).cpu().numpy()
                if self.reuse_plan:
                    # store previous actions for next iteration
                    self.prev_plan = curr_plan[self.apply_plan_step:]
                ...
                struct.pack_into(
                    "d" * self.act_dim * self.apply_plan_step, ctrl_data, 8, *ctrl_act.flatten())
                ctrl_buffer[:] = ctrl_data

                for k, v in curr_info.items():
                    log_queue.put(
                        {"tag": f"Planning/{k}", "value": v, "step": step})
    ...
    def eval_process(self, manager_dict, log_queue):
        ...
        self.env = instantiate(self.eval_env_config, device=self.device)
        self.env.set_is_evaluating(self.config.command)
        obs = self.env.reset_all()["actor_obs"]
        init_buf = self.env.get_mppi_buffers(
            [0])  # get buffer for the environment
        ...
        ctrl_shm = shared_memory.SharedMemory(
            name="ctrl_shm", create=True, size=ctrl_shm_size)
        ctrl_buffer = ctrl_shm.buf
        # Shared Memory for state variables
        env_num_dof = self.env.num_dofs
        env_root_state_dim = self.env.robot_root_states.shape[-1]
        obs_dim = obs.shape[-1]
        state_shm_size = 8 + env_num_dof * 2 * 8 + env_root_state_dim * \
            8 + obs_dim * 8  # time + dof_state + root_state + obs
        ...
        try:
            try:
                ...
                while not done:
                    ...
                    struct.pack_into("d" * (env_num_dof * 2), state_data, 8, *
                                     curr_state["dof_states"].to(torch.float64).cpu().numpy().flatten())
                    struct.pack_into("d" * env_root_state_dim, state_data, 8 + env_num_dof * 2 * 8,
                                     *curr_state["root_states"].to(torch.float64).cpu().numpy().flatten())
                    struct.pack_into("d" * obs_dim, state_data, 8 + env_num_dof * 2 * 8 +
                                     env_root_state_dim * 8, *curr_state["obs"].to(torch.float64).cpu().numpy().flatten())
                    state_buffer[:] = state_data

                    curr_buf = self.env.get_mppi_buffers(
                        [0])  # get buffer for the environment
                    buf_data = bytearray(buf_shm_size)
                    buf_size = 0
                    for k, v in curr_buf.items():
                        struct.pack_into("d" * v.numel(), buf_data, buf_size, *v.to(
                            # pack buffer data
                            torch.float64).cpu().numpy().flatten())
                        buf_size += v.numel() * 8
                    buf_buffer[:] = buf_data
                    ...
                    actions = torch.frombuffer(ctrl_data[8:], dtype=torch.float64).to(
                        torch.float32).view(self.apply_plan_step, self.act_dim).to(self.device)

                    for i in range(self.apply_plan_step):
                        stored_actions.append(actions[[i]].cpu().numpy())
                        stored_states.append(curr_state)
                        nxt_full_ob, reward, dones, info = self.env.step(
                            actions[[i]])
                        done = dones[0]
                        cum_rew += reward[0]
                        t_curr = time.time() - t0
                        step += 1
                        print(
                            f"at step {step} cumulated reward {cum_rew}; planning time {t_curr:.2f} seconds")
                        log_queue.put(
                            {"tag": "Eval/reward_per_step", "value": reward[0], "step": step})
                        log_queue.put(
                            {"tag": "Eval/cumulative_reward", "value": cum_rew, "step": step})
                        log_queue.put({"tag": "Eval/eval_time",
                                      "value": t_curr, "step": step})
                        if done:
                            break
                ...
                if self.config.log_actions:
                    pkl.dump(stored_actions, open(
                        f"{self.log_dir}/data/stored_actions_{self.config.eval_steps}step_{self.config.command}.pkl", "wb"))
                if self.config.log_states:
                    pkl.dump(stored_states, open(
                        f"{self.log_dir}/data/stored_states_{self.config.eval_steps}step_{self.config.command}.pkl", "wb"))
    ...
    def log_process(self, wandb_config, manager_dict, log_queue):
        ...
        while True:
            ...
            if log_data is None:  # End logging when receiving None
                break
            writer.add_scalar(
                log_data["tag"], log_data["value"], log_data["step"])
        writer.close()
    ...
    def evaluate_policy(self, wandb_config=None):
        with Manager() as manager:
            ...
            eval_proc = Process(target=self.eval_process,
                                args=(manager_dict, log_queue))
            act_proc = Process(target=self.act_process,
                               args=(manager_dict, log_queue))
            log_proc = Process(target=self.log_process, args=(
                wandb_config, manager_dict, log_queue))
