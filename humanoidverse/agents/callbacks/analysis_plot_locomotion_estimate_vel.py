    ...
    def _update_plot(self):
        ...
        def add_trace(x, y, color, row, col, name=None, show_legend=False, line_width=2):
            fig.add_trace(go.Scatter(x=x, y=y, line=dict(
                color=color, width=line_width), name=name, showlegend=show_legend), row=row, col=col)
        ...
        add_trace(time, log["estimated_vel_x"], YELLOW,
                  1, 1, "Estimated vel x", line_width=1)
        add_trace(time, log["estimated_vel_y"], YELLOW,
                  1, 2, "Estimated vel y", line_width=1)
        add_trace(time, log["estimated_vel_z"], YELLOW,
                  1, 3, "Estimated vel z", line_width=1)

        # Vertical Contact forces
        forces = log["contact_forces_z"]
        for i in range(forces[0].shape[0]):
            add_trace(time, [force[i]
                      for force in forces], BLUE, 2, 1, f"Force {i}")
