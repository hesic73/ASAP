...
class AnalysisPlotMotionTrackingOpenloop(RL_EvalCallback):
    ...
    def on_pre_eval_env_step(self, actor_state):
        ...
        if self.log_single_robot:
            open_loop_actions = self.env.get_open_loop_action_at_current_timestep() * \
                self.env.config.robot.control.action_scale
    ...
    def on_post_eval_env_step(self, actor_state):
        ...
        if self.compute_metrics:
            self.gt_pos.append(
                self.env.extras['ref_body_pos_extend'].cpu().numpy())
            self.gt_rot.append(
                self.env.extras['ref_body_rot_extend'].cpu().numpy())
            ...
            self.mpjpe.append(
                self.env.dif_global_body_pos.norm(dim=-1).cpu() * 1000)
            ...
            termination_state = torch.logical_and(
                self.curr_stpes <= self.env._motion_lib.get_motion_num_steps() - 1, died)
            ...
            if self.curr_stpes >= curr_max or self.terminate_state.sum() == self.env.num_envs:
                ...
                self.terminate_memory.append(
                    self.terminate_state.cpu().numpy())
                self.success_rate = (1 - np.concatenate(self.terminate_memory)
                                     [: self.env._motion_lib._num_unique_motions].mean())

                # MPJPE
                all_mpjpe = torch.stack(self.mpjpe)
                try:
                    # Max should be the same as the number of frames in the motion.
                    assert (all_mpjpe.shape[0] == curr_max or self.terminate_state.sum(
                    ) == self.env.num_envs)
                except:
                    import ipdb
                    ipdb.set_trace()
                ...
                all_mpjpe = [all_mpjpe[: (i - 1), idx].mean() for idx, i in enumerate(
                    self.env._motion_lib.get_motion_num_steps())]
                all_body_pos_pred = [all_body_pos_pred[: (
                    i - 1), idx] for idx, i in enumerate(self.env._motion_lib.get_motion_num_steps())]
                all_body_pos_gt = [all_body_pos_gt[: (
                    i - 1), idx] for idx, i in enumerate(self.env._motion_lib.get_motion_num_steps())]
                all_body_rot_pred = [all_body_rot_pred[: (
                    i - 1), idx] for idx, i in enumerate(self.env._motion_lib.get_motion_num_steps())]
                all_body_rot_gt = [all_body_rot_gt[: (
                    i - 1), idx] for idx, i in enumerate(self.env._motion_lib.get_motion_num_steps())]
                ...
                if (self.env.start_idx + self.env.num_envs >= self.env._motion_lib._num_unique_motions):
                    ...
                    succ_idxes = np.nonzero(
                        ~terminate_hist[: self.env._motion_lib._num_unique_motions])[0].tolist()

                    pred_pos_all_succ = [(self.pred_pos_all[:self.env._motion_lib._num_unique_motions])[
                        i] for i in succ_idxes]
                    gt_pos_all_succ = [(self.gt_pos_all[: self.env._motion_lib._num_unique_motions])[
                        i] for i in succ_idxes]
                    pred_rot_all_succ = [(self.pred_rot_all[: self.env._motion_lib._num_unique_motions])[
                        i] for i in succ_idxes]
                    gt_rot_all_succ = [(self.gt_rot_all[: self.env._motion_lib._num_unique_motions])[
                        i] for i in succ_idxes]
                    ...
                    pred_rot_all = self.pred_rot_all[:
                                                     self.env._motion_lib._num_unique_motions]
                    ...
                    metrics_all_print = {m: np.mean(
                        v) for m, v in metrics.items()}
                    metrics_print = {m: np.mean(v)
                                     for m, v in metrics_succ.items()}
                    ...
                    print("Succ: ", " \t".join(
                        [f"{k}: {v:.3f}" for k, v in metrics_print.items()]))
...
class WebLogger:
    ...
    def _update_plot(self):
        ...
        for i in range(num_dofs):
            ...
            add_trace(time, [pos[i] for pos in log["dof_open_loop_actions"]],
                      PURPLE, row, 1, f"DOF {i} open loop actions")
            add_trace(time, [pos[i] for pos in log["dof_perfect_delta_a"]],
                      GREEN, row, 1, f"DOF {i} perfect delta a")
            add_limit_lines(
