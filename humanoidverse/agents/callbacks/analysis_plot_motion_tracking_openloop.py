...
class AnalysisPlotMotionTrackingOpenloop(RL_EvalCallback):
    ...
    def on_post_eval_env_step(self, actor_state):
        ...
        if self.compute_metrics:
            ...
            self.mpjpe.append(
                self.env.dif_global_body_pos.norm(
                    dim=-1).cpu() * 1000)

            died = actor_state['dones']
            died[actor_state['extras']["time_outs"]] = False

            # if terminate after the last frame, then it is not a termination.
            # curr_step is one step behind simulation.
            ...
            if self.curr_stpes >= curr_max or self.terminate_state.sum() == self.env.num_envs:
                ...
                try:
                    # Max should be the same as the number of frames in the
                    # motion.
                    assert (
                        all_mpjpe.shape[0] == curr_max or self.terminate_state.sum() == self.env.num_envs)
                ...
                if (self.env.start_idx + self.env.num_envs >=
                        self.env._motion_lib._num_unique_motions):
            ...
            update_str = f"Terminated: {
                self.terminate_state.sum().item()} | max frames: {curr_max} | steps {
                self.curr_stpes} | Start: {
                self.env.start_idx} | Succ rate: {
                self.success_rate:.3f} | Mpjpe: {
                    np.mean(
                        self.mpjpe_all) *
                1000:.3f}"
    ...
    def _update_plot(self):
        ...
        for i in range(num_dofs):
            ...
            add_trace(time,
                      ...
                      f"DOF {i} pos target")
            add_trace(time,
                      [pos[i] for pos in log["dof_open_loop_actions"]],
                      PURPLE,
                      row,
                      1,
                      f"DOF {i} open loop actions")
            add_trace(time,
                      [pos[i] for pos in log["dof_perfect_delta_a"]],
                      GREEN,
                      row,
                      1,
                      f"DOF {i} perfect delta a")
            add_limit_lines(
