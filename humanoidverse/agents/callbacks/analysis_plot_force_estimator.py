    ...
    def _update_plot(self):
        ...
        def get_subplot_titles():
            ...
            for i in range(num_dofs):
                titles.extend([f'DOF {i} Position', f'DOF {i} Velocity',
                              f'DOF {i} Torque', f'DOF {i} Torque/Velocity'])
            return titles
        ...
        def add_trace(x, y, color, row, col, name=None, show_legend=False):
            fig.add_trace(go.Scatter(x=x, y=y, line=dict(
                color=color), name=name, showlegend=show_legend), row=row, col=col)

        add_trace(time, log["vr_3point_pos"][:, 0, 0],
                  BLUE, 1, 1, "Left Hand Position x")
        add_trace(time, log["vr_3point_pos_target"][:, 0, 0],
                  RED, 1, 1, "Left Hand Position Target x")
        # add_trace(time, log["left_hand_force"][:, 0] * force_to_position_scale + force_to_position_bias, RED, 1, 1, "Left Hand Force x")

        add_trace(time, log["vr_3point_pos"][:, 0, 1],
                  BLUE, 1, 2, "Left Hand Position y")
        add_trace(time, log["vr_3point_pos_target"][:, 0, 1],
                  RED, 1, 2, "Left Hand Position Target y")
        # add_trace(time, log["left_hand_force"][:, 1] * force_to_position_scale + force_to_position_bias, RED, 1, 2, "Left Hand Force y")

        add_trace(time, log["vr_3point_pos"][:, 0, 2],
                  BLUE, 1, 3, "Left Hand Position z")
        add_trace(time, log["vr_3point_pos_target"][:, 0, 2],
                  RED, 1, 3, "Left Hand Position Target z")
        # add_trace(time, log["left_hand_force"][:, 2] * force_to_position_scale + force_to_position_bias, RED, 1, 3, "Left Hand Force z")

        add_trace(time, np.linalg.norm(
            log["left_hand_force"], axis=1), RED, 1, 4, "Left Hand Force")

        add_trace(time, log["vr_3point_pos"][:, 1, 0],
                  BLUE, 2, 1, "Right Hand Position x")
        add_trace(time, log["vr_3point_pos_target"][:, 1, 0],
                  RED, 2, 1, "Right Hand Position Target x")
        # add_trace(time, log["right_hand_force"][:, 0] * force_to_position_scale + force_to_position_bias, RED, 2, 1, "Right Hand Force x")

        add_trace(time, log["vr_3point_pos"][:, 1, 1],
                  BLUE, 2, 2, "Right Hand Position y")
        add_trace(time, log["vr_3point_pos_target"][:, 1, 1],
                  RED, 2, 2, "Right Hand Position Target y")
        # add_trace(time, log["right_hand_force"][:, 1] * force_to_position_scale + force_to_position_bias, RED, 2, 2, "Right Hand Force y")

        add_trace(time, log["vr_3point_pos"][:, 1, 2],
                  BLUE, 2, 3, "Right Hand Position z")
        add_trace(time, log["vr_3point_pos_target"][:, 1, 2],
                  RED, 2, 3, "Right Hand Position Target z")
        # add_trace(time, log["right_hand_force"][:, 2] * force_to_position_scale + force_to_position_bias, RED, 2, 3, "Right Hand Force z")

        add_trace(time, np.linalg.norm(
            log["right_hand_force"], axis=1), RED, 2, 4, "Right Hand Force")

        add_trace(time, log["vr_3point_pos"][:, 2, 0],
                  BLUE, 3, 1, "Head Position x")
        add_trace(time, log["vr_3point_pos_target"][:, 2, 0],
                  BLACK, 3, 1, "Head Position Target x")

        add_trace(time, log["vr_3point_pos"][:, 2, 1],
                  BLUE, 3, 2, "Head Position y")
        add_trace(time, log["vr_3point_pos_target"][:, 2, 1],
                  BLACK, 3, 2, "Head Position Target y")

        add_trace(time, log["vr_3point_pos"][:, 2, 2],
                  BLUE, 3, 3, "Head Position z")
        add_trace(time, log["vr_3point_pos_target"][:, 2, 2],
                  BLACK, 3, 3, "Head Position Target z")

        # Vertical Contact forces
        forces = log["contact_forces_z"]
        for i in range(forces[0].shape[0]):
            add_trace(time, [force[i]
                      for force in forces], BLUE, 3, 4, f"Force {i}")

        add_trace(time, log["left_hand_force"][:, 2],
                  BLUE, 3, 4, "Left Hand Force z")
        add_trace(time, log["right_hand_force"][:, 2],
                  RED, 3, 4, "Right Hand Force z")

        # error

        add_trace(time, log["vr_3point_pos_target"][:, 0, 0] -
                  log["vr_3point_pos"][:, 0, 0], RED, 4, 1, "Left Hand Error x")
        add_trace(time, log["vr_3point_pos_target"][:, 0, 1] -
                  log["vr_3point_pos"][:, 0, 1], RED, 4, 2, "Left Hand Error y")
        add_trace(time, log["vr_3point_pos_target"][:, 0, 2] -
                  log["vr_3point_pos"][:, 0, 2], RED, 4, 3, "Left Hand Error z")
        add_trace(time, np.linalg.norm(log["vr_3point_pos_target"][:, 0] -
                  log["vr_3point_pos"][:, 0], axis=1), RED, 4, 4, "Left Hand Error")

        add_trace(time, log["vr_3point_pos_target"][:, 1, 0] -
                  log["vr_3point_pos"][:, 1, 0], RED, 5, 1, "Right Hand Error x")
        add_trace(time, log["vr_3point_pos_target"][:, 1, 1] -
                  log["vr_3point_pos"][:, 1, 1], RED, 5, 2, "Right Hand Error y")
        add_trace(time, log["vr_3point_pos_target"][:, 1, 2] -
                  log["vr_3point_pos"][:, 1, 2], RED, 5, 3, "Right Hand Error z")
        add_trace(time, np.linalg.norm(log["vr_3point_pos_target"][:, 1] -
                  log["vr_3point_pos"][:, 1], axis=1), RED, 5, 4, "Right Hand Error")

        add_trace(time, log["left_hand_force"][:, 0],
                  RED, 6, 1, "Left Hand Force x")
        add_trace(time, log["left_force_estimated"][:, 0],
                  YELLOW, 6, 1, "Estimated Left Hand Force x")

        add_trace(time, log["left_hand_force"][:, 1],
                  RED, 6, 2, "Left Hand Force y")
        add_trace(time, log["left_force_estimated"][:, 1],
                  YELLOW, 6, 2, "Estimated Left Hand Force y")

        add_trace(time, log["left_hand_force"][:, 2],
                  RED, 6, 3, "Left Hand Force z")
        add_trace(time, log["left_force_estimated"][:, 2],
                  YELLOW, 6, 3, "Estimated Left Hand Force z")

        add_trace(time, np.linalg.norm(
            log["left_hand_force"], axis=1), RED, 6, 4, "Left Hand Force")
        add_trace(time, np.linalg.norm(
            log["left_force_estimated"], axis=1), YELLOW, 6, 4, "Estimated Left Hand Force")

        add_trace(time, log["right_hand_force"][:, 0],
                  RED, 7, 1, "Right Hand Force x")
        add_trace(time, log["right_force_estimated"][:, 0],
                  YELLOW, 7, 1, "Estimated Right Hand Force x")
        add_trace(time, log["right_hand_force"][:, 1],
                  RED, 7, 2, "Right Hand Force y")
        add_trace(time, log["right_force_estimated"][:, 1],
                  YELLOW, 7, 2, "Estimated Right Hand Force y")
        add_trace(time, log["right_hand_force"][:, 2],
                  RED, 7, 3, "Right Hand Force z")
        add_trace(time, log["right_force_estimated"][:, 2],
                  YELLOW, 7, 3, "Estimated Right Hand Force z")
        add_trace(time, np.linalg.norm(
            log["right_hand_force"], axis=1), RED, 7, 4, "Right Hand Force")
        add_trace(time, np.linalg.norm(
            log["right_force_estimated"], axis=1), YELLOW, 7, 4, "Estimated Right Hand Force")
        ...
        for i in range(num_dofs):
            ...
            # Position
            add_trace(time, [pos[i] for pos in log["dof_pos"]],
                      BLUE, row, 1, f"DOF {i} pos")
            add_trace(time, [pos[i] for pos in log["dof_pos_target"]],
                      RED, row, 1, f"DOF {i} pos target")
            add_limit_lines(
                row, 1, self.log_dof_pos_limits[i, 0], self.log_dof_pos_limits[i, 1])
            # Velocity
            add_trace(time, [vel[i] for vel in log["dof_vel"]],
                      BLUE, row, 2, f"DOF {i} vel")
            add_limit_lines(
                row, 2, -self.log_dof_vel_limits[i], self.log_dof_vel_limits[i])
            # Torque
            add_trace(time, [torque[i] for torque in log["dof_torque"]],
                      BLUE, row, 3, f"DOF {i} torque")
            add_limit_lines(
                row, 3, -self.log_dof_torque_limits[i], self.log_dof_torque_limits[i])

            add_trace(time, [torque[i] for torque in log["feedforward_residual_tau"]],
                      RED, row, 3, f"Feedforward Residual Torque")
            add_trace(time, [torque[i] for torque in log["feedback_residual_tau"]],
                      BLACK, row, 3, f"Feedback Residual Torque")
            ...
            # Add torque limits
            fig.add_shape(type="line", x0=-self.log_dof_vel_limits[i], y0=-self.log_dof_torque_limits[i],
                          x1=self.log_dof_vel_limits[i], y1=-
                          self.log_dof_torque_limits[i],
                          line=dict(color=YELLOW, width=2), row=row, col=4)
            fig.add_shape(type="line", x0=-self.log_dof_vel_limits[i], y0=self.log_dof_torque_limits[i],
                          x1=self.log_dof_vel_limits[i], y1=self.log_dof_torque_limits[i],
                          line=dict(color=YELLOW, width=2), row=row, col=4)

        fig.update_layout(height=300*num_rows, width=1500,
                          title_text="Robot State Plots", showlegend=True)
        ...
        fig.update_yaxes(
            title_text="Vertical Contact Forces [N]", row=3, col=4)
