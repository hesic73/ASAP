...
class AnalysisPlotForceEstimator(RL_EvalCallback):
    ...
    def on_pre_evaluate_policy(self):
        ...
        self.logger.set_robot_limits(
            self.log_dof_pos_limits,
            self.log_dof_vel_limits,
            self.log_dof_torque_limits)
    ...
    def _update_plot(self):
        ...
        def get_subplot_titles():
            ...
            for i in range(num_dofs):
                titles.extend([f'DOF {i} Position', f'DOF {i} Velocity', f'DOF {
                              i} Torque', f'DOF {i} Torque/Velocity'])
            return titles
        ...
        add_trace(time,
                  np.linalg.norm(log["vr_3point_pos_target"][:,
                                                             0] - log["vr_3point_pos"][:,
                                                                                       0],
                                 axis=1),
                  RED,
        ...
        add_trace(
            time,
            np.linalg.norm(
                log["left_force_estimated"],
                axis=1),
            YELLOW,
            6,
            4,
            "Estimated Left Hand Force")
        ...
        add_trace(
            time,
            np.linalg.norm(
                log["right_force_estimated"],
                axis=1),
            YELLOW,
            7,
            4,
            "Estimated Right Hand Force")
        ...
        for i in range(num_dofs):
            ...
            add_trace(time,
                      [pos[i] for pos in log["dof_pos_target"]],
                      RED,
                      row,
                      1,
                      f"DOF {i} pos target")
            ...
            # Torque
            add_trace(time,
                      [torque[i] for torque in log["dof_torque"]],
                      BLUE,
                      row,
                      3,
                      f"DOF {i} torque")
            ...
            add_trace(time,
                      [torque[i] for torque in log["feedforward_residual_tau"]],
                      RED,
                      row,
                      3,
                      f"Feedforward Residual Torque")
            add_trace(time,
                      [torque[i] for torque in log["feedback_residual_tau"]],
                      BLACK,
                      row,
                      3,
                      f"Feedback Residual Torque")
        ...
        fig.update_yaxes(
            title_text="Vertical Contact Forces [N]",
            row=3,
            col=4)
