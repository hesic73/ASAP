...
class WebLogger:
    ...
    def _update_plot(self):
        ...
        add_trace(time, log["left_hand_force"][:, 0],
                  RED, 4, 1, "Left Hand Force x")
        add_trace(time, log["left_hand_force"][:, 1],
                  RED, 4, 2, "Left Hand Force y")
        add_trace(time, log["left_hand_force"][:, 2],
                  RED, 4, 3, "Left Hand Force z")
        add_trace(time, np.linalg.norm(
            log["left_hand_force"], axis=1), RED, 4, 4, "Left Hand Force")

        add_trace(time, log["right_hand_force"][:, 0],
                  RED, 5, 1, "Right Hand Force x")
        add_trace(time, log["right_hand_force"][:, 1],
                  RED, 5, 2, "Right Hand Force y")
        add_trace(time, log["right_hand_force"][:, 2],
                  RED, 5, 3, "Right Hand Force z")
        add_trace(time, np.linalg.norm(
            log["right_hand_force"], axis=1), RED, 5, 4, "Right Hand Force")
