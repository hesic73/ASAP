...
class AnalysisPlotMotionTracking(RL_EvalCallback):
    ...
    def __init__(self, config, training_loop: PPO):
        ...
        self.use_weblogger = self.config.use_weblogger if hasattr(
            self.config, 'use_weblogger') else True
        ...
        # if genesis, only print the results of the second iteration of
        # evaluation
    ...
    def on_post_eval_env_step(self, actor_state):
        ...
        if self.use_weblogger and (
                step + 1) % self.config.plot_update_interval == 0:
