...
class PPOForceEstimator(PPO):
    ...
    def _setup_models_and_optimizer(self):
        ...
        self.left_hand_force_estimator = Estimator(
            self.algo_obs_dim_dict,
            self.config.module_dict.left_hand_force_estimator).to(
            self.device)
        self.right_hand_force_estimator = Estimator(
            self.algo_obs_dim_dict,
            self.config.module_dict.right_hand_force_estimator).to(
            self.device)
        self.optimizer_left_hand_force_estimator = torch.optim.Adam(
            self.left_hand_force_estimator.parameters(),
            lr=self.config.estimator_learning_rate)
        self.optimizer_right_hand_force_estimator = torch.optim.Adam(
            self.right_hand_force_estimator.parameters(),
            lr=self.config.estimator_learning_rate)
            ...
            if self.load_optimizer:
                ...
                logger.info(
                    f"Critic Learning rate: {
                        self.critic_learning_rate}")
                logger.info(
                    f"Left Hand Force Estimator Learning rate: {
                        self.estimator_learning_rate}")
                logger.info(
                    f"Right Hand Force Estimator Learning rate: {
                        self.estimator_learning_rate}")
    ...
    def _actor_act_step(self, obs_dict):
        ...
        actor_input = torch.cat([actor_obs,
    ...
    def _update_estimator(self, policy_state_dict, loss_dict):
        ...
        left_hand_force_estimator_loss = F.mse_loss(
            left_hand_force_estimator_output,
            left_hand_force_estimator_target)
        ...
        right_hand_force_estimator_loss = F.mse_loss(
            right_hand_force_estimator_output,
            right_hand_force_estimator_target)
