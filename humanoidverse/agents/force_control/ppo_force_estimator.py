...
class PPOForceEstimator(PPO):
    ...
    def load(self, ckpt_path):
        if ckpt_path is not None:
            ...
            self.left_hand_force_estimator.load_state_dict(
                loaded_dict["left_hand_force_estimator_model_state_dict"])
            self.right_hand_force_estimator.load_state_dict(
                loaded_dict["right_hand_force_estimator_model_state_dict"])
            if self.load_optimizer:
                self.actor_optimizer.load_state_dict(
                    loaded_dict["actor_optimizer_state_dict"])
                self.critic_optimizer.load_state_dict(
                    loaded_dict["critic_optimizer_state_dict"])
                self.optimizer_left_hand_force_estimator.load_state_dict(
                    loaded_dict["left_hand_force_estimator_optimizer_state_dict"])
                self.optimizer_right_hand_force_estimator.load_state_dict(
                    loaded_dict["right_hand_force_estimator_optimizer_state_dict"])
                ...
                self.estimator_learning_rate = loaded_dict[
                    'left_hand_force_estimator_optimizer_state_dict']['param_groups'][0]['lr']
                self.set_learning_rate(
                    self.actor_learning_rate, self.critic_learning_rate, self.estimator_learning_rate)
                ...
                logger.info(
                    f"Left Hand Force Estimator Learning rate: {self.estimator_learning_rate}")
                logger.info(
                    f"Right Hand Force Estimator Learning rate: {self.estimator_learning_rate}")
    ...
    def _setup_storage(self):
        ...
        self.storage.register_key('left_hand_force_estimator_output', shape=(
            self.algo_obs_dim_dict["left_hand_force_estimator_target"],), dtype=torch.float)
        self.storage.register_key('right_hand_force_estimator_output', shape=(
            self.algo_obs_dim_dict["right_hand_force_estimator_target"],), dtype=torch.float)
    ...
    def _actor_act_step(self, obs_dict):
        ...
        left_hand_force_estimator_output = self.left_hand_force_estimator(
            long_history).detach()
        right_hand_force_estimator_output = self.right_hand_force_estimator(
            long_history).detach()
        actor_obs = obs_dict["actor_obs"]
        actor_input = torch.cat(
            [actor_obs, left_hand_force_estimator_output, right_hand_force_estimator_output], dim=-1)
    ...
    def _update_estimator(self, policy_state_dict, loss_dict):
        ...
        left_hand_force_estimator_output = self.left_hand_force_estimator(
            policy_state_dict["long_history_for_estimator"])
        left_hand_force_estimator_loss = F.mse_loss(
            left_hand_force_estimator_output, left_hand_force_estimator_target)

        right_hand_force_estimator_target = policy_state_dict["right_hand_force_estimator_target"]
        right_hand_force_estimator_output = self.right_hand_force_estimator(
            policy_state_dict["long_history_for_estimator"])
        right_hand_force_estimator_loss = F.mse_loss(
            right_hand_force_estimator_output, right_hand_force_estimator_target)

        self.optimizer_left_hand_force_estimator.zero_grad()
        left_hand_force_estimator_loss.backward()
        nn.utils.clip_grad_norm_(
            self.left_hand_force_estimator.parameters(), self.max_grad_norm)
        ...
        nn.utils.clip_grad_norm_(
            self.right_hand_force_estimator.parameters(), self.max_grad_norm)
    ...
    def _pre_eval_env_step(self, actor_state: dict):
        left_hand_force_estimator_output = self.left_hand_force_estimator(
            actor_state["obs"]['long_history_for_estimator'])
        right_hand_force_estimator_output = self.right_hand_force_estimator(
            actor_state["obs"]['long_history_for_estimator'])
        actor_state.update(
            {"left_hand_force_estimator_output": left_hand_force_estimator_output})
        actor_state.update(
            {"right_hand_force_estimator_output": right_hand_force_estimator_output})
        input_for_actor = torch.cat(
            [actor_state["obs"]['actor_obs'], left_hand_force_estimator_output, right_hand_force_estimator_output], dim=-1)
