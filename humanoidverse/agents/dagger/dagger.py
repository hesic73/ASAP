...
class DAgger(BaseAlgo):
    ...
    def setup(self):
        ...
        self.actor = PPOActorFixSigma(
            self.algo_obs_dim_dict, actor_network_dict, {}, self.num_act)
        self.gt_actor = PPOActorFixSigma(
            self.algo_obs_dim_dict, teacher_actor_network_dict, teacher_actor_network_load_dict, self.num_act)
        ...
        self.storage.register_key('gt_actions', shape=(
            self.num_act,), dtype=torch.float)
        self.storage.register_key(
            'gt_actions_log_prob', shape=(1,), dtype=torch.float)
        self.storage.register_key('gt_action_mean', shape=(
            self.num_act,), dtype=torch.float)
    ...
    def _post_epoch_logging(self, log_dict, width=80, pad=35):
        ...
        self.writer.add_scalar('Loss/learning_rate',
                               self.learning_rate, log_dict['it'])
        self.writer.add_scalar('Loss/mean_bc_loss',
                               log_dict['mean_bc_loss'], log_dict['it'])
        self.writer.add_scalar('Policy/mean_noise_std',
                               mean_std.item(), log_dict['it'])
    ...
    def get_example_obs(self):
        ...
        for obs_key in obs_dict.keys():
            logger.info(
                f"{obs_key}, {sorted(self.env.config.obs.obs_dict[obs_key])}")
        # move to cpu
        for k in obs_dict:
